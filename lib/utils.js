...
function findApkIconPath (info) {
  ...
  if (Object.keys(resultMap).length === 0 || !maxDpiIcon.icon) {
    for (const iconIndex in info.application.icon) {
      let iconPath = info.application.icon[iconIndex]
      // 解决部分应用无法获取应用图标的问题，如果：微信
      let fileExtension = iconPath.substring(iconPath.lastIndexOf('.'))
